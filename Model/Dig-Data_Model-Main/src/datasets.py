...
def getEmotionDict() -> Dict[str, int]:
    ...
    # return {'ang': 0, 'dis': 1, 'fea': 2, 'hap': 3, 'sad': 4, 'sur': 5, 'neu': 6}
    return {'ang': 0, 'dis': 1, 'fea': 2, 'hap': 3, 'sad': 4, 'sur': 5}
    # return {'ang': 0, 'hap': 1, 'sad': 2, 'sur': 3}
    # return {'ang-sad': 0, 'hap-sur': 1}
# for classificate three categories
    # return {'ang': 0, 'exc': 1, 'fru': 0, 'hap': 1, 'neu': 0, 'sad': 0}

def get_dataset_iemocap(data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    main_folder = os.path.join(data_folder, 'BIG_DATA_RAW_PROCESSED_FACE')
    meta = load(os.path.join(main_folder, 'meta_one_hot_label_six_categories.pkl'))

    # meta = load(os.path.join('/home/matt/Dig-Data_Model-Main/data/LIRIS_RAW_PROCESSED_Face/LIRIS_meta/meta_9.pkl'))
    # print('load meta_9.pkl')

    emoDict = getEmotionDict()
    print(f'Final_{phase}_split_six_categories_eNTERFACE’05.txt')
    uttr_ids = open(os.path.join(data_folder, 'data_split', 'all_single_label_six_category', 'with_valid', f'Final_{phase}_split_six_categories_eNTERFACE’05.txt'), 'r').read().splitlines()
    # uttr_ids = open(os.path.join(data_folder, 'data_split', 'all_single_label_six_category', 'with_valid', 'NoisyLabel', 'O2Unet', f'Final_{phase}_split_six_categories_no_mosei_no_LIRIS_remove_5p_noise_O2U.txt'), 'r').read().splitlines()
    
    texts = [meta[uttr_id]['text'] for uttr_id in uttr_ids]
    # for uttr_id in uttr_ids:
    #     if meta[uttr_id]['label'] == [0, 0, 0, 0, 1, 0]:
    #         meta[uttr_id]['label'] = 'sad'
    #     elif meta[uttr_id]['label'] == [0, 0, 1, 0, 0, 0]:
    #         meta[uttr_id]['label'] = 'fea'
    # labels = [emoDict[meta[uttr_id]['label']] for uttr_id in uttr_ids]
    labels = [meta[uttr_id]['label'] for uttr_id in uttr_ids]
    # labels = []
    # for uttr_id in uttr_ids:
    #     if meta[uttr_id]['label'] == [1, 0, 0, 0, 0, 0]:
    #         labels.append([1, 0, 0, 0])
    #         # labels.append([1, 0])
    #     elif meta[uttr_id]['label'] == [0, 0, 0, 1, 0, 0]:
    #         labels.append([0, 1, 0, 0])
    #         # labels.append([0, 1])
    #     elif meta[uttr_id]['label'] == [0, 0, 0, 0, 1, 0]:
...
# for classificate three categories
            # label_annotations=['ang-fru-neu-sad', 'neu-sad'],
...
class IEMOCAP(Dataset):
    ...
    def sample_imgs_by_interval(self, folder: str, fps: Optional[int] = 30) -> List[str]:
        ...
        #     sampled = [os.path.join(folder, f'image_{i}.jpg') for i in list(range(0, nums, step))]
        step = int(self.img_interval / 1000 * fps)
        sampled = [os.path.join(folder, f'image_{i}.jpg') for i in list(range(0, nums, int(step)))]
        return sampled
    ...
    def getPosWeight(self):
        '''
            total data count summary
            ang: 9453
            dis: 5299
            fea: 3388 + 8 = 3396
            hap: 16044
            sad: 10003 + 1841 = 11844
            sur: 6597
            neu: 13738
        '''
        # from collections import Counter
        # print(Counter((self.labels_no_onehot).numpy()))
        # weight = compute_class_weight(class_weight='balanced', classes=np.array([0, 1, 2, 3]), y=self.labels_no_onehot.numpy())
        # weight = weight * 2
        # print(weight)
        # return weight
        # y = [0]*9453 + [1]*5299 + [2]*3396 + [3]*16044 + [4]*11844 + [5]*6597 + [6]*13738
        # y = [0]*3267 + [1]*2738 + [2]*1263 + [3]*7587 + [4]*4026 + [5]*1465
        # y = [0]*8414 + [1]*4983 + [2]*3026 + [3]*16072 + [4]*10632 + [5]*5627+ [6]*12192 #without liris&ravdess_02
        # y = np.array(y)
        # weight = compute_class_weight(class_weight='balanced', classes=np.array([0, 1, 2, 3, 4, 5]), y=y)
        # # weight = compute_class_weight(class_weight='balanced', classes=np.array([0, 1, 2, 3, 4, 5, 6]), y=y)
        # print(weight)
        # return weight
        ...
        neg_nums = self.__len__() - pos_nums
        pos_weight = neg_nums / pos_nums
        # pos_weight = np.array([4.63600783, 5.67826087, 18.65870307, 1.24780488, 3.5443787, 18.86206897])
        # pos_weight = np.array([4.18414322*1.6, 8.79227053*1.6, 0.91859915, 4.44161074])
    ...
    def __getitem__(self, ind: int) -> Tuple[str, np.array, List[torch.tensor], List[int], np.array]:
        ...
        # waveform, sr = torchaudio.load(os.path.join('/home/matt/1227/BIG_DATA_RAW_PROCESSED_FACE(AudiobyDeepfilternet)/', uttrId, f'audio_16000.wav'))

        # print('waveform: ', waveform.shape)
        # print('sample rate: ', sr, type(sr))
        # Cut WAV
        ...
        # Cut Spec
        waveform = waveform.mean(dim=0, keepdim=True)  # 將 shape [2, num_samples] -> [1, num_samples]
        specgram = torchaudio.transforms.MelSpectrogram(sample_rate=sr, win_length=int(float(sr) / 44100 * 400))(waveform).unsqueeze(0)
        # specgram = torchaudio.transforms.MelSpectrogram(sample_rate=sr, win_length=int(float(sr) / 16000 * 400))(waveform).unsqueeze(0)
        # print('uttrId: ', uttrId, 'specgram lenght: ', specgram.size())
        specgrams = self.cutSpecToPieces(specgram)
        # print(torch.cat(specgrams, dim=0).shape)

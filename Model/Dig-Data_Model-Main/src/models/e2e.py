...
class MME2E(nn.Module):
    ...
    def forward(self, imgs, imgs_lens, specs, spec_lens, text):
        ...
        if 't' in self.mod:
            ...
            # all_logits.append(torch.zeros_like(self.t_out(text_cls)))
        ...
        # return all_logits[0] + all_logits[1] + all_logits[2], all_logits[0], all_logits[1], all_logits[2]
        # return self.weighted_fusion(torch.stack(all_logits, dim=-1)).squeeze(-1), all_logits[0], all_logits[1], all_logits[2]
    ...
    def embeddings(self, imgs, imgs_lens, specs, spec_lens, text):
        all_logits = []
        ...
        # return self.t_out(text_cls), self.v_out(faces), self.a_out(specs)
        return all_logits[0], all_logits[1], all_logits[2]
        # return self.weighted_fusion(torch.stack(all_logits, dim=-1)).squeeze(-1)
    

    def SHAP_feature(self, imgs, imgs_lens, specs, spec_lens, text):
        all_logits = []
